    ...
    function validateTime(id) {
        ...
        if (timeValue) {
            ...
            if (hour < 7 || (hour >= 22 && minute > 0)) {
                M.toast({html: 'Selecciona una hora entre 07:00 y 22:00.', classes: 'red darken-1'});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var especializadoDetalle = document.getElementById('especializado-detalle');

    selectEquipo.addEventListener('change', function() {
        if (selectEquipo.value === 'especializado') {
            especializadoDetalle.style.display = 'block';
        } else {
            especializadoDetalle.style.display = 'none';
        }
    });
});
